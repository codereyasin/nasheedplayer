...
const DashboardUser = () => {
  ...
  return (
    <div className="w-fulll p-4 flex items-center justify-center flex-col">
      <div className="relative w-full py-12 min-h-[400px] overflow-x-scroll my-4 flex flex-col items-center justify-start p-4 border border-gray-300 rounded-md gap-3">
        ...
        {allUsers &&
          allUsers?.map((data, index) => (
            <DashboardUserCard data={data} key={index} index={index} />
          ))}
      </div>
    </div>
  );
     ...
     {data._id !== user?.user._id && ( 
      ...
      <div className="w-275 min-w-[160px] text-center flex items-center justify-center gap-6 relative">
        ...
        {isUserRoleUpdated && (
          ...
          className="absolute z-10 top-6 right-4 rounded-md p-4 flex items-start flex-col gap-4 bg-white shadow-xl max-w-[159px]">
            <p className="text-textColor text-[12px] font-semibold">
              Are You sure, do want to mark the user as 
            ...
            <div className="flex justify-center items-center gap-4 ">
              ...
              <motion.button
                whileTap={{ scale: 0.75 }}
                className="outline-none border-none text-sm px-4 py-1 rounded-md bg-gray-400 text-black hover:shadow-md"
