...
const DashboardnewNasheed = () => {
  const [nasheedName, setNasheedName] = useState("");
  ...
  const [nasheedImageCover, setNasheedImageCover] = useState(null);
  ...
  const deleFileObject = (url, isImage) => {
    if (isImage) {
  ...
  const saveNasheed = async () => {
    if (!nasheedImageCover || !audioImageCover) {
      dispath({
        type: actionType.SET_ALERT_TYPE,
        alertType: "danger",
      });
      ...
      const data = {
        name: nasheedName,
        imageURL: nasheedImageCover,
        nasheedURL: audioImageCover,
        album: albumFilter,
        artist: artistFilter,
        language: languageFilter,
        category: filterTerm,
      };
      await saveNewNasheed(data).then((res) => {
        getAllNasheed().then((nasheed) => {
          dispath({
            type: actionType.SET_ALL_NASHEED,
            allNasheed: nasheed.nasheed,
          });
        });
      });
      dispath({
      ...
      dispath({ type: actionType.SET_FILTER_TERN, filterTerm: null });
    }
  };

  const saveArtist = async () => {
    if (!artistImageCover || !artistName || !facebook || !youtube) {
      dispath({
        type: actionType.SET_ALERT_TYPE,
        alertType: "danger",
      });
      ...
      await saveNewArtist(data).then((res) => {
        getAllArtists().then((data) => {
          dispath({
            type: actionType.SET_ALL_ARTISTS,
            allArtists: data.artist,
  ...
  const saveAlbum = async () => {
    if (!albumImageCover || !albumName) {
      ...
      await saveNewAlbum(data).then((res) => {
        getAllAlbums().then((data) => {
          dispath({
            type: actionType.SET_ALL_ALBUMNS,
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 border border-gray-300 rounded-md gap-4  ">
      <input
        type="text"
        placeholder="Type your Nasheed name..."
        className="w-full p-3 rounded-md text-base font-semibold text-textColor outline-none shadow-sm border border-gray-300 bg-transparent"
        value={nasheedName || ""}
        onChange={(e) => setNasheedName(e.target.value)}
      />
      ...
      <div className="bg-card backdrop-blur-md w-full h-300 rounded-md border-2 border-dotted border-gray-300 cursor-pointer">
        ...
        {!isImageLoading && (
          <>
            {!nasheedImageCover ? (
              <FileUploader
                updateState={setNasheedImageCover}
              ...
              <div className="relative w-full h-full overflow-hidden rounded-md">
                ...
                <button
                  ...
                  onClick={() => deleFileObject(nasheedImageCover, true)}
                >
        ...
        {!isAudioLoading && (
          <>
            {!audioImageCover ? (
              ...
              <div className="relative w-full h-full flex overflow-hidden rounded-md items-center justify-center">
                <audio src={audioImageCover} controls />
                <button
                  type="button"
                  className="absolute bottom-3 right-3 p-3 rounded-full bg-red-500 text-xl cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out"
                  onClick={() => deleFileObject(audioImageCover, false)}
                >
                  <MdDelete className="text-white" />
                </button>
              </div>
        ...
        {!isArtistUploading && (
          <>
            {!artistImageCover ? (
              ...
              <div className="relative w-full h-full flex overflow-hidden rounded-md items-center justify-center">
                <audio src={artistImageCover} controls />
                <button
                  type="button"
                  className="absolute bottom-3 right-3 p-3 rounded-full bg-red-500 text-xl cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out"
                  onClick={() => deleFileObject(artistImageCover, true)}
                >
                  <MdDelete className="text-white" />
                </button>
              </div>
        ...
        {!isAlbumUploading && (
          <>
            {!albumImageCover ? (
              ...
              <div className="relative w-full h-full flex overflow-hidden rounded-md items-center justify-center">
                <img src={albumImageCover} alt="" />
                <button
                  type="button"
                  className="absolute bottom-3 right-3 p-3 rounded-full bg-red-500 text-xl cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out"
                  onClick={() => deleFileObject(albumImageCover, true)}
                >
                  <MdDelete className="text-white" />
                </button>
              </div>
      ...
      <input
        ...
        className="w-full text-base font-semibold border border-gray-300 p-3 rounded-lg text-textColor outline-none bg-transparent"
        value={albumName}
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      ...
      <div className="w-20 h-20 min-w-[40px] bg-red-600 animate-ping rounded-full flex items-center justify-center relative">
        <div className="w-20 h-20 min-w-[40px] bg-red-600 blur-xl"></div>
      </div>
    </div>
  );
};
...
export const FileUploader = ({
  ...
  const uploadFile = (e) => {
    ...
    const storageRef = ref(
      storage,
      `${isImage ? "Images" : "Nasheed"}/${Date.now()}-${uploadedFile.name}`
    );
...
export default DashboardnewNasheed;
