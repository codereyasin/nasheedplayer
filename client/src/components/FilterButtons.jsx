...
const FilterButtons = ({filterData, flag}) => {
  ...
  const updateFilterButton = (name) =>{
    ...
    if(flag === 'Category'){
      dispatch({
        type: actionType.SET_FILTER_TERN,
        filterTerm: name
      })
    }
  }
    ...
    <div className='border border-gray-300 rounded-md px-4 py-1 relative cursor-pointer hover:border-gray-400'>
      <p onClick={()=> setFilterMenu(!filterMenu)} className='text-base tracking-wide text-textColor flex items-center gap-2'>
        ...
        {filterData && filterMenu && (
           ...
           exit={{ opacity: 0, y: 50 }}
           className='w-48 z-50 backdrop-blur-sm max-h-44 overflow-y-scroll scrollbar-thin scrollbar-track-gray-200 scrollbar-thumb-gray-400 py-2 flex-col rounded-md shadow-md absolute top-8 left-0'>
            {filterData?.map(data =>(
              <div 
               key={data.name}
